    ...
    stage('Build Javari') {
     ...
     stage('Deploy on EC2') {
       steps {
         ...
         sh "scp ./**/target/*App.jar ${sshUser}@${sshServer}/home/build/javari/back-end "
         ...
         sh "sshpass -p ${sshPassword} ssh ${sshUser}@${sshServer} mv /opt/applications/javari*.jar /opt/applications/bkp/ "
         echo "Copy new Jar to Applications folder"
         sh "sshpass -p ${sshPassword} ssh ${sshUser}@${sshServer} cp /home/build/javari/back-end/javari*.jar /opt/applications/bkp/ "
         echo "Add permission to exec"
         sh "sshpass -p ${sshPassword} ssh ${sshUser}@${sshServer} chmod +x /opt/applications/*.jar "
         sh "sshpass -p ${sshPassword} ssh ${sshUser}@${sshServer} chown dteodoro /opt/applications/*.jar " 
         echo "Start services" 
         ...
         sh "sshpass -p ${sshPassword} ssh ${sshUser}@${sshServer} rm -rf /home/build/javari/back-end/*.jar "
       }
     }
  }
}
